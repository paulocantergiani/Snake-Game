...
class SnakeGame:
    def __init__(self):
        ...
        self.score = 0
        self.scoreboard = self.scoreboard_setup()
    ...
    def scoreboard_setup(self):
        ...
        scoreboard.goto(0, SCREEN_HEIGHT // 2 - 20)
        scoreboard.write(f"SCORE: {self.score}")
        return scoreboard
    ...
    def move_apple(self, apple):
        while True:
            ...
            if [x, y] not in [
                (segment.xcor(), segment.ycor()) for segment in self.snake
            ]:
                apple.goto(x, y)
                break
    ...
    def check_collision(self):
        ...
        # Wall collision
        if abs(x) > (SCREEN_WIDTH / 2 - SQUARE_SIZE / 2) or abs(y) > (
        ...
        while True:
            ...
            if self.check_collision():
                ...
                answer = self.screen.textinput(
                    "Game Over",
                    f"YOU LOST! YOUR SCORE WAS {self.score}. Would you like to play again? (Yes/No): ",
                )
            ...
            print(
                f"Snake head position: x={self.snake[0].xcor()}, y={self.snake[0].ycor()}"
            )
